...
def registerNext():
	try:
		...
		user = Login(firstname=request.form["fname"], lastname=request.form["lname"], email=request.form["email"],rollNo=request.form["rollNo"],password=request.form['loginpassword'], messFile=messFileName)
		db.session.add(user)
		db.session.commit()
	except:
